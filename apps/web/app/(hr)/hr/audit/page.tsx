...
import { useHrAuditLog } from '@/hooks/useHr';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import type { HrAuditLogRecord } from '@cdy/shared';
import { cn } from '@/lib/utils';

function actionColor(action: string): string {
  if (action.endsWith('.created')) return 'text-blue-400';
  if (action.endsWith('.updated') || action.endsWith('.completed')) {
    return 'text-amber-400';
  }
  if (action.endsWith('.deleted') || action.endsWith('.terminated')) {
    return 'text-cdy-red';
  }
  return 'text-cdy-muted';
}

function formatActionDetails(log: HrAuditLogRecord): string {
  if (log.action === 'review.created' && log.newValue) {
    const nv = log.newValue as { period?: string };
    return nv.period ? `Review — ${nv.period}` : 'Performance review created';
  }
  if (log.action === 'review.completed' && log.newValue) {
    const nv = log.newValue as { overallRating?: number; period?: string };
    const parts: string[] = [];
    if (nv.period) parts.push(nv.period);
    if (nv.overallRating != null) parts.push(`Rating: ${nv.overallRating}/5`);
    return parts.join(' · ') || 'Review completed';
  }
  if (log.action === 'employee.created' && log.newValue) {
    const nv = log.newValue as { firstName?: string; lastName?: string };
    if (nv.firstName || nv.lastName) {
      return `${nv.firstName ?? ''} ${nv.lastName ?? ''}`.trim();
    }
  }
...
export default function HrAuditPage(): JSX.Element {
  ...
  const [diffLog, setDiffLog] = useState<HrAuditLogRecord | null>(null);

  const { data, isLoading, isError } = useHrAuditLog({
    page,
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-2xl font-semibold text-cdy-white">HR Audit Log</h1>
      ...
      {diffLog && (
        <DiffModal log={diffLog} onClose={() => setDiffLog(null)} />
      )}
    </div>
  );
}
