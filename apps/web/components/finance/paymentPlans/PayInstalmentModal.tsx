...
interface PayInstalmentModalProps {
  open: boolean;
  onClose: () => void;
  plan: PaymentPlanRecord;
  instalment: PaymentPlanInstalment;
  currency: string;
  invoiceId: string;
}
...
export function PayInstalmentModal({
  ...
  currency,
  invoiceId,
}: PayInstalmentModalProps): JSX.Element | null {
  ...
  async function handleSubmit(e: React.FormEvent): Promise<void> {
    ...
    try {
      await api.post<ApiResponse<unknown>>(
        `/payment-plans/${plan.id}/instalments/${instalment.id}/pay`,
        {
          paidAt,
          method,
      ...
      toast.success(
        `Instalment ${instalment.instalmentNumber} marked as paid`,
      );
      ...
      toast.error(
        axiosErr.response?.data?.message ?? 'Failed to record instalment payment',
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="mb-4 text-sm text-cdy-muted">
            Instalment #{instalment.instalmentNumber} — {fmt(instalment.amount)}
          </p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="instPaidAt">Payment date</Label>
              <Input
                id="instPaidAt"
                type="date"
                value={paidAt}
            ...
            <div className="space-y-2">
              <Label htmlFor="instMethod">Method</Label>
              <select
                id="instMethod"
                value={method}
            ...
            <div className="space-y-2">
              <Label htmlFor="instReference">Reference (optional)</Label>
              <Input
                id="instReference"
                value={reference}
