...
export const SYSTEM_FEATURES = [
  ...
  { key: 'finance.tax.lookup', name: 'Tax Rates — Lookup', module: 'finance', description: 'Pick active tax rates in forms (not full tax admin)' },
  ...
  { key: 'finance.accounts', name: 'Company Accounts', module: 'finance', description: 'Manage company bank/mobile money receiving accounts' },
  { key: 'finance.accounts.lookup', name: 'Company Accounts — Lookup', module: 'finance', description: 'Pick active company accounts in forms (not full account admin)' },
  ...
  { key: 'ventures.lookup', name: 'Ventures — Lookup', module: 'finance', description: 'Search/pick ventures for forms (not full ventures finance access)' },
  ...
  { key: 'hr.departments.lookup', name: 'Departments — Lookup', module: 'hr', description: 'Pick departments in forms (not full HR directory)' },
  ...
  { key: 'projects.lookup', name: 'Projects — Lookup', module: 'projects', description: 'Search/pick projects for linking (not full projects access)' },
...
export const DEFAULT_ROLES: Array<{
  ...
  },
  {
    ...
    permissions: [
      ...
      { key: 'finance.tax.lookup', canRead: true, canWrite: false },
      ...
      { key: 'finance.accounts', canRead: true, canWrite: true },
      { key: 'finance.accounts.lookup', canRead: true, canWrite: false },
      { key: 'finance.reserve', canRead: true, canWrite: true },
      { key: 'ventures.view', canRead: true, canWrite: false },
      { key: 'ventures.lookup', canRead: true, canWrite: false },
      ...
      { key: 'hr.departments.lookup', canRead: true, canWrite: false },
      ...
      { key: 'projects.lookup', canRead: true, canWrite: false },
...
export async function seedRbac(prisma: PrismaClient): Promise<void> {
  ...
  // These roles only need venture pickers — not full ventures finance UI.
  // Existing DBs may still have ventures.view; revoke in favor of ventures.lookup.
  await prisma.rolePermission.deleteMany({
    where: {
      role: { key: { in: ['SALES_AGENT', 'TEAM_MEMBER', 'PROJECT_MANAGER'] } },
      feature: { key: 'ventures.view' },
    },
  });
